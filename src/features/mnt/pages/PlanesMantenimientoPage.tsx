/**
 * Planes de Mantenimiento MNT — Listado y gestión. GET/POST/PUT /api/v1/mnt/planes-mantenimiento
 */
import { useState, useEffect, useCallback } from 'react';
import { toast } from 'react-hot-toast';
import { Loader, Calendar, Plus, Pencil } from 'lucide-react';
import { activoService } from '../services/mnt.service';
import { planMantenimientoService } from '../services/mnt.service';
import type { Activo } from '../types/mnt.types';
import type { PlanMantenimiento, PlanMantenimientoCreate, PlanMantenimientoUpdate } from '../types/mnt.types';
...
const TIPOS_MANT = ['preventivo', 'predictivo'] as const;
const FRECUENCIA_TIPO = ['dias', 'horas_uso', 'kilometros', 'ciclos'] as const;

const DEFAULT: PlanMantenimientoCreate = {
  activo_id: '',
  codigo_plan: '',
  nombre: '',
  tipo_mantenimiento: 'preventivo',
  frecuencia_tipo: 'dias',
  frecuencia_valor: 30,
  es_activo: true,
};
...
export default function PlanesMantenimientoPage() {
  ...
  const [list, setList] = useState<PlanMantenimiento[]>([]);
  ...
  const [editing, setEditing] = useState<PlanMantenimiento | null>(null);
  const [form, setForm] = useState<PlanMantenimientoCreate>(DEFAULT);
  const [editForm, setEditForm] = useState<PlanMantenimientoUpdate>({});
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const data = await planMantenimientoService.list(
  ...
  const openEdit = (row: PlanMantenimiento) => {
    setEditing(row);
    setEditForm({
      codigo_plan: row.codigo_plan,
      nombre: row.nombre,
      descripcion: row.descripcion ?? undefined,
      tipo_mantenimiento: row.tipo_mantenimiento as PlanMantenimientoCreate['tipo_mantenimiento'],
      frecuencia_tipo: row.frecuencia_tipo as PlanMantenimientoCreate['frecuencia_tipo'],
      frecuencia_valor: row.frecuencia_valor,
      fecha_ultimo_mantenimiento: row.fecha_ultimo_mantenimiento ?? undefined,
      fecha_proximo_mantenimiento: row.fecha_proximo_mantenimiento ?? undefined,
      tareas_mantenimiento: row.tareas_mantenimiento ?? undefined,
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await planMantenimientoService.create(form);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await planMantenimientoService.update(editing.plan_mantenimiento_id, editForm);
  ...
  return (
    <MntPageLayout
      ...
      description="Programa de mantenimientos preventivos y predictivos por activo."
      action={
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Frecuencia</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Próximo</th>
                <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Acciones</th>
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
              {list.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500 dark:text-gray-400"><Calendar className="h-10 w-10 mx-auto mb-2 opacity-50" />No hay planes de mantenimiento.</td></tr>
              ) : (
                list.map((row) => (
                  <tr key={row.plan_mantenimiento_id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{row.frecuencia_valor} {row.frecuencia_tipo}</td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{row.fecha_proximo_mantenimiento ?? '—'}</td>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><Label>Tipo *</Label><select value={form.tipo_mantenimiento} onChange={(e) => setForm((p) => ({ ...p, tipo_mantenimiento: e.target.value as PlanMantenimientoCreate['tipo_mantenimiento'] }))} className={selectCls} required>{TIPOS_MANT.map((t) => <option key={t} value={t}>{t}</option>)}</select></div>
              <div><Label>Frecuencia tipo *</Label><select value={form.frecuencia_tipo} onChange={(e) => setForm((p) => ({ ...p, frecuencia_tipo: e.target.value as PlanMantenimientoCreate['frecuencia_tipo'] }))} className={selectCls} required>{FRECUENCIA_TIPO.map((f) => <option key={f} value={f}>{f}</option>)}</select></div>
              <div><Label>Frecuencia valor *</Label><input type="number" min={1} value={form.frecuencia_valor || ''} onChange={(e) => setForm((p) => ({ ...p, frecuencia_valor: parseInt(e.target.value, 10) || 0 }))} className={inputCls} required /></div>
              <div><Label>Fecha próximo</Label><input type="date" value={form.fecha_proximo_mantenimiento ?? ''} onChange={(e) => setForm((p) => ({ ...p, fecha_proximo_mantenimiento: e.target.value || undefined }))} className={inputCls} /></div>
              <div><Label>Costo estimado</Label><input type="number" step="0.01" min={0} value={form.costo_estimado ?? ''} onChange={(e) => setForm((p) => ({ ...p, costo_estimado: e.target.value ? parseFloat(e.target.value) : undefined }))} className={inputCls} /></div>
            </div>
      ...
      <Dialog open={editOpen} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><Label>Código *</Label><input type="text" value={editForm.codigo_plan ?? ''} onChange={(e) => setEditForm((p) => ({ ...p, codigo_plan: e.target.value }))} className={inputCls} required /></div>
              ...
              <div><Label>Frecuencia valor *</Label><input type="number" min={1} value={editForm.frecuencia_valor ?? ''} onChange={(e) => setEditForm((p) => ({ ...p, frecuencia_valor: parseInt(e.target.value, 10) || 0 }))} className={inputCls} required /></div>
              <div><Label>Fecha próximo</Label><input type="date" value={editForm.fecha_proximo_mantenimiento ?? ''} onChange={(e) => setEditForm((p) => ({ ...p, fecha_proximo_mantenimiento: e.target.value || undefined }))} className={inputCls} /></div>
            </div>
