...
export const conexionService = {
   ...
   */
  async getConexionPrincipal(cliente_id: string, modulo_id: string): Promise<Conexion | null> {
    try {
   ...
   */
  async updateConexion(conexion_id: string, conexionData: ConexionUpdate): Promise<Conexion> {
    try {
   ...
   */
  async deleteConexion(conexion_id: string): Promise<void> {
    try {
