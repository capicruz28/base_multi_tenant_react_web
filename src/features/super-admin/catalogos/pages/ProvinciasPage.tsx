...
const ProvinciasPage: React.FC = () => {
  ...
  const [deleteTarget, setDeleteTarget] = useState<CatProvincia | null>(null);
  ...
  const fetchList = useCallback(async () => {
    ...
    setError(null);
    try {
      const params = departamentoFilter ? { departamento_id: departamentoFilter } : {};
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      await catalogosGlobalService.deleteProvincia(deleteTarget.provincia_id);
  ...
  return (
    <div className="w-full">
      <div className="mb-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={openCreate} disabled={departamentos.length === 0} className="flex items-center gap-2 px-4 py-2 bg-brand-primary text-white rounded-lg hover:bg-brand-primary-hover focus:ring-2 focus:ring-brand-primary focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        {loading && (
          <div className="flex justify-center items-center py-12">
            ...
            <span className="ml-2 text-gray-600 dark:text-gray-400">Cargando provincias...</span>
          </div>
        )}
        {error && !loading && (
          <div className="p-6">
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredList.length === 0 ? (
                  <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-500 dark:text-gray-400"><MapPin className="h-10 w-10 mx-auto mb-2 opacity-50" />{list.length === 0 ? 'No hay provincias.' : 'No hay resultados para la búsqueda.'}</td></tr>
                ) : (
                  filteredList.map((row) => (
                  <tr key={row.provincia_id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{departamentoNombre(row.departamento_id)}</td>
          ...
          <form onSubmit={handleCreate} className="flex flex-col min-h-0 flex-1">
            <DialogBody className="px-6 pb-2">
              <div className="space-y-4">
                <div><Label>Departamento *</Label><select value={form.departamento_id} onChange={(e) => setForm((p) => ({ ...p, departamento_id: e.target.value }))} className={selectClass} required><option value="">Seleccionar</option>{departamentos.map((d) => <option key={d.departamento_id} value={d.departamento_id}>{d.nombre}</option>)}</select></div>
      ...
      <Dialog open={editOpen} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-lg max-h-[90vh] flex flex-col gap-0 p-0">
          ...
          <form onSubmit={handleUpdate} className="flex flex-col min-h-0 flex-1">
            <DialogBody className="px-6 pb-2">
              <div className="space-y-4">
                <div><Label>Departamento *</Label><select value={editForm.departamento_id ?? ''} onChange={(e) => setEditForm((p) => ({ ...p, departamento_id: e.target.value }))} className={selectClass} required><option value="">Seleccionar</option>{departamentos.map((d) => <option key={d.departamento_id} value={d.departamento_id}>{d.nombre}</option>)}</select></div>
      ...
      <ConfirmDialog isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDeleteConfirm} title="Eliminar provincia" message={deleteTarget ? `¿Eliminar la provincia "${deleteTarget.nombre}"?` : ''} confirmText="Eliminar" cancelText="Cancelar" variant="danger" loading={deleting} />
    </div>
  );
};
